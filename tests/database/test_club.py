...
def test_create_clube(setup_database):
    clube = Clube(
        ...
        descricao='Um clube para amantes de literatura clássica brasileira'
    )
    db.session.add(clube)
    db.session.commit()

    clube_criado = db.session.get(Clube, 'C004')
...
def test_read_clube(setup_database):
    clube = db.session.get(Clube, 'C004')
...
def test_update_clube(setup_database):
    ...
    clube_atualizado = db.session.get(Clube, 'C004')
...
def test_delete_clube(setup_database):
    ...
    clube_excluido = db.session.get(Clube, 'C004')
...
def test_create_participa(setup_database):
    ...
    participa_criado = db.session.get(Participa, ('1234567890', 'C004'))
    assert participa_criado is not None
    assert participa_criado.clube_id == 'C004'

# Teste para ler participação
def test_read_participa(setup_database):
    participa = db.session.get(Participa, ('1234567890', 'C004'))
...
def test_delete_participa(setup_database):
    ...
    participa_excluida = db.session.get(Participa, ('1234567890', 'C004'))
...
def test_create_adiciona(setup_database):
    ...
    adiciona_criado = db.session.get(Adiciona, ('1234567890', 'C004', 'L124'))
...
def test_read_adiciona(setup_database):
    adiciona = db.session.get(Adiciona, ('1234567890', 'C004', 'L124'))
...
def test_delete_adiciona(setup_database):
    ...
    adiciona_excluido = db.session.get(Adiciona, ('1234567890', 'C004', 'L124'))
